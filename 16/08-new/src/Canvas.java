...
public class Canvas extends JPanel {
    ...
    // File I/O members
    public Canvas(BufferedReader br) throws IOException {
        if(!br.readLine().equals("Canvas")) 
            throw new IOException("Missing 'Canvas' ID");
        color = new Color(Integer.parseInt(br.readLine()));
        stroke = new BasicStroke(Integer.parseInt(br.readLine()));
        int size = Integer.parseInt(br.readLine());
        while(size-- > 0) {
            String shapeID = br.readLine();
            if      (shapeID.equals(Shape.id()))     shapes.add(new Shape(br));
            else if (shapeID.equals(Rectangle.id())) shapes.add(new Rectangle(br));
            else if (shapeID.equals(Plot.id()))      shapes.add(new Plot(br));
            else if (shapeID.equals(Turtle.id()))    shapes.add(new Turtle(br));
            else if (shapeID.equals(Polygon.id()))   shapes.add(new Polygon(br));
            else throw new IOException("Invalid shape ID " + shapeID);

/*          Fails because id() is not a "constant String expression"  :-(  
            switch(shapeID) {
                case Shape.id()     : shapes.add(new Shape(br));     break;
                case Rectangle.id() : shapes.add(new Rectangle(br)); break;
                case Plot.id()      : shapes.add(new Plot(br));      break;
                case Turtle.id()    : shapes.add(new Turtle(br));    break;
                case Polygon.id()   : shapes.add(new Polygon(br));   break;
                default: throw IOException("Invalid shape ID " + shapeID);
            }
*/
        }
    }
