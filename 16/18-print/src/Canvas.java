...
public class Canvas extends JPanel {
    ...
    public enum PenMode {SEGMENT, CONTIGUOUS, FREEHAND, TEXT};
    ...
    public Canvas(BufferedReader br, String fileVersion) throws IOException {
        ...
        while(size-- > 0) {
            ...
            if      (shapeID.equals(Shape.id()))     addShape(new Shape(br, fileVersion));
            else if (shapeID.equals(Rectangle.id())) addShape(new Rectangle(br, fileVersion));
            else if (shapeID.equals(Plot.id()))      addShape(new Plot(br, fileVersion));
            else if (shapeID.equals(Turtle.id()))    addShape(new Turtle(br, fileVersion));
            else if (shapeID.equals(Polygon.id()))   addShape(new Polygon(br, fileVersion));
...
/*
            ...
            switch(shapeID) {
                case Shape.id()     : addShape(new Shape(br, fileVersion));     break;
                case Rectangle.id() : addShape(new Rectangle(br, fileVersion)); break;
                case Plot.id()      : addShape(new Plot(br, fileVersion));      break;
                case Turtle.id()    : addShape(new Turtle(br, fileVersion));    break;
                case Polygon.id()   : addShape(new Polygon(br, fileVersion));   break;
                default: throw IOException("Unknown shape ID: " + shapeID);
            }
*/
        }
        nextShape = shapes.size();
    }
    ...
    public void save(BufferedWriter bw) throws IOException {
        ...
        for(int i=0; i<nextShape; ++i) shapes.get(i).save(bw); // Tell shapes to save themselves
    ...
    public void onMouseClicked(MouseEvent event) {
        ...
        if ((penMode != PenMode.FREEHAND) &&
            ...
            if(penMode == PenMode.TEXT) {
                addShape(new Text(text, x2, y2, color));
                clickNumber = 0;
            }
            ...
            if (clickNumber == 2) { // second click
                addShape(new Shape(      // add 1st segment
            ...
            if (clickNumber >= 3) { 
                if(x1 != x2 || y1 != y2) { 
                    shapes.get(nextShape-1).addPoint(x2, y2);
                } else {
                    clickNumber = 0;
                    rubberBand = null;
                    setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
    ...
    public void addShape(Shape shape) {
        if(nextShape < shapes.size()) // clear redo buffer
            shapes.subList(nextShape, shapes.size()).clear();
        shapes.add(shape);
        nextShape = shapes.size();
    }
    protected void clearRedoBuffer() {
        if(nextShape < shapes.size()) shapes.subList(nextShape, shapes.size()).clear();
    }
    public void undo() {
        if(nextShape > 0) {--nextShape; repaint();}
    }
    public void redo() {
        if(shapes.size() > nextShape) {++nextShape; repaint();}
    }
