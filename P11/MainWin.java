...
public class MainWin extends JFrame {
    ...
    public MainWin(String title) {
        ...
        JButton buttonListAnimals  = new JButton(new ImageIcon("resources/list_available.png"));
          buttonListAnimals.setActionCommand("List Animals");
          buttonListAnimals.setToolTipText("List animals available for adoption");
          setBorder(buttonListAnimals);
          toolbar.add(buttonListAnimals);
          buttonListAnimals.addActionListener(event -> updateDisplay(DataView.ANIMALS));
        ...
        JButton buttonListClients  = new JButton(new ImageIcon("resources/list_clients.png"));
          buttonListClients.setActionCommand("List Animals");
          buttonListClients.setToolTipText("List animals available for adoption");
          setBorder(buttonListClients);
          toolbar.add(buttonListClients);
          buttonListClients.addActionListener(event -> updateDisplay(DataView.CLIENTS));

        // A "horizontal strut" is just a space of the specified pixel width
        toolbar.add(Box.createHorizontalStrut(24));
        
        JButton buttonAdopt  = new JButton(new ImageIcon("resources/adopt.png"));
          buttonAdopt.setActionCommand("Adopt");
          buttonAdopt.setToolTipText("Adopt (match a Client and Animal)");
          setBorder(buttonAdopt);
          toolbar.add(buttonAdopt);
          buttonAdopt.addActionListener(event -> onAdoptAnimalClick());
        
        toolbar.add(Box.createHorizontalStrut(4));

        JButton buttonListAdoptions  = new JButton(new ImageIcon("resources/list_adoptions.png"));
          buttonListAdoptions.setActionCommand("List Adoptions");
          buttonListAdoptions.setToolTipText("List animals who have been adopted");
          setBorder(buttonListAdoptions);
          toolbar.add(buttonListAdoptions);
          buttonListAdoptions.addActionListener(event -> updateDisplay(DataView.ADOPTIONS));
...
/*        for(int i=0; i<shelter.numClients(); ++i)
            cbClient.addItem(shelter.getClient(i));
*/        
        Object[] objects = { // Array of widgets to display
            animal, cbAnimal, client, cbClient};
    ...
    private void updateDisplay(DataView view) {
        ...
        String result = "Invalid display selected: " + view;
        if(view == DataView.ANIMALS)   result = shelter.toString();
        if(view == DataView.CLIENTS)   result = shelter.clientsToString();
        if(view == DataView.ADOPTIONS) result = shelter.adoptionsToString();
        if(result.length() > 0) {
